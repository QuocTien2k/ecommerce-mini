...
type CreateSettingFormProp = {
  open: boolean;
  onClose: () => void;
};

const AdminCreateSetting = ({ open, onClose }: CreateSettingFormProp) => {
  const form = useAdminCreateSettingForm();
    ...
    async (values: CreateSettingFormOutput) => {
      ...
      try {
        const result = await run(() =>
          createSettingMutation.mutateAsync({
            data: {
  ...
  return (
    ...
    >
      <div
        className=" max-h-[95vh] w-full max-w-4xl overflow-y-auto rounded-2xl
        border border-white/10 bg-white p-6 shadow-2xl
      "
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-lg font-semibold">Tạo cài đặt website</h2>
...
export default AdminCreateSetting;
